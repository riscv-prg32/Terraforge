...
firmware_elf="${1:-"${PRG32_FIRMWARE_ELF:-"${BUILD_DIR:-"$prg32_repo/build"}/PRG32.elf"}"}"
...
Update PRG32 to a branch/release with the portable ABI-table tooling.
...
python3 "$game_tool" build \
  ...
  --name terraforge32 \
  --out "$repo_dir/dist/terraforge32-$architecture.raw.prg32" \
  "${build_args[@]}"

python3 "$game_tool" attach-metadata \
  "$repo_dir/dist/terraforge32-$architecture.raw.prg32" \
  --out "$repo_dir/dist/terraforge32-$architecture.prg32" \
...
echo "$repo_dir/dist/terraforge32-$architecture.prg32"
